...
def configure_adherent_drug(cb, cost=1, drug="DHA", dont_allow_duplicates=1,
                            dosing_type="FullTreatmentCourse", adherence_config={}, non_adherence_options=["NEXT_UPDATE"],
                            non_adherence_distribution=[1], max_dose_consideration_duration=3.40282e+38,
    ...
    """
        ...
        adherent drug currently takes only one drug at a time, the name needs to be specified
    ...
    if not adherence_config:
        adherence_config = {  # the default is for person to take everything
            "class": "WaningEffectMapLinearAge",
            "Initial_Effect": 1.0,
            "Durability_Map":
                {
...
def add_adherent_drug(config_builder, start=1, coverage=1, cost=1, nodeIDs=[], number_repetitions=1,
                      timesteps_between_repititions=0, drug="DHA", dont_allow_duplicates=1,
                      dosing_type="FullTreatmentCourse", adherence_config={}, non_adherence_options=["NEXT_UPDATE"],
    ...
    """
    # built-in default so we can run this function by just putting in the config builder.
    adherent_drug = configure_adherent_drug(config_builder, cost, drug, dont_allow_duplicates,
                      dosing_type, adherence_config, non_adherence_options,
                      non_adherence_distribution, max_dose_consideration_duration, took_dose_event)
